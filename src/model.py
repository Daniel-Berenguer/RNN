...
class BatchNorm(nn.Module):
    def __init__(self, nout):
        ...
        self.runningMean = torch.zeros(nout)
        self.runningStd = torch.ones(nout)
    ...
    def forward(self, X, train=True):
        if train:
            ...
            with torch.no_grad():
                ...
                self.runningMean = 0.99 * self.runningMean + 0.01 * batchMean
                self.runningStd = 0.99 * self.runningStd + 0.01 * batchStd
...
class GRU_Layer(nn.Module):
    def __init__(self, nin, hSize, bN=True):
        super().__init__()
        self.hSize = hSize
        self.bN = bN
        ...
        # Output Calc
        self.V = nn.Parameter(torch.randn(hSize, 1))
